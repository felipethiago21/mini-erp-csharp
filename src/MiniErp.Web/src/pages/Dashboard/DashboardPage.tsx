    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <h2 className="mb-3 text-sm font-semibold text-slate-900">Produtos com menor estoque</h2>
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <h2 className="mb-3 text-sm font-semibold text-slate-900">Vendas mais recentes</h2>
                ...
                <ul className="divide-y divide-slate-100">
                  {data.vendasMaisRecentes.map((venda) => (
                    <li key={venda.id} className="flex items-center justify-between py-2.5 text-sm">
                      <div>
                        <p className="font-medium text-slate-900">{venda.clienteNome}</p>
                        <p className="text-slate-500">{formatarDataHora(venda.data)}</p>
                      </div>
                      <div className="text-right">
                        <CurrencyDisplay value={venda.total} className="font-medium text-slate-900" />
          ...
          {isFetching ? <p className="text-xs text-slate-400">Atualizando...</p> : null}
        </div>
      ) : null}
    </div>
  )
